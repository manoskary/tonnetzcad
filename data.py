...
def get_dataset(args, path=None, verbose=True, download=True, regenerate=False, **kwargs):
    ...
    assert not (download and regenerate), "You can either download the o.g. Mozart Annotated Sonatas dataset or generate your own - but not both"
    try:
        if regenerate:
            raise ValueError("Regenerating dataset") # yes this is hacky
        if download:
            print("Downloading the Mozart Annotated Sonatas dataset from {}".format(args.url))
